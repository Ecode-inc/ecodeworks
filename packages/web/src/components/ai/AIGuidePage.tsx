...
const getActions = [
  ...
  },
  {
    category: '📅 캘린더',
    items: [
      { path: '/action/list-events', desc: '일정 조회', params: 'start, end, context (group=공유만/private=개인포함)' },
  ...
  },
  {
    ...
    items: [
      ...
      { path: '/action/trash-doc', desc: '문서를 휴지통으로 이동', params: 'id (문서ID)' },
      { path: '/action/restore-doc', desc: '휴지통에서 복원', params: 'id (문서ID)' },
      { path: '/action/list-trash-docs', desc: '휴지통 목록 조회', params: '(없음)' },
  ...
  },
  {
    category: '\uD83D\uDDBC\uFE0F 문서 이미지',
    items: [
      { path: '/action/list-doc-images', desc: '문서 이미지 목록', params: 'document_id (선택), tag (태그 필터), person (인물 필터)' },
