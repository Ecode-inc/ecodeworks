...
documentsRoutes.get('/', async (c) => {
  ...
  let query = 'SELECT id, department_id, parent_id, title, is_folder, order_index, created_by, created_at, updated_at FROM documents WHERE 1=1'
  ...
  query += ' ORDER BY is_folder DESC, order_index ASC'
...
documentsRoutes.post('/', requirePermission('docs', 'write'), async (c) => {
  ...
  await c.env.DB.prepare(
    `INSERT INTO documents (id, department_id, parent_id, title, content, is_folder, order_index, created_by)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?)`
  ).bind(id, deptId, body.parent_id || null, body.title, body.content || '', body.is_folder ? 1 : 0, (maxOrder?.max_idx ?? -1) + 1, user.id).run()
...
documentsRoutes.patch('/:id', authMiddleware, async (c) => {
  const user = c.get('user')
  const docId = c.req.param('id')
  const body = await c.req.json<{ title?: string; content?: string; parent_id?: string }>()
...
documentsRoutes.get('/search', async (c) => {
  ...
  // FTS5 search
  let query = `
    SELECT d.id, d.department_id, d.parent_id, d.title, d.is_folder, d.created_at, d.updated_at,
